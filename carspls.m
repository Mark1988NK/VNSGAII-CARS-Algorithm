...
% sampling runs does not have signicant influence on the performance of
% CARS Thus, the number of sampling runs was set to 100 as default during the calculation process.
...
for iter=2:num
     ...
     ratio=a*exp(-b*iter);                      %+++ Ratio of retained variables.
     Ratio(iter)=ratio; 
     K=round(Nx*ratio);
...
for i=1:num
   ...
   RMSEP(i)=CV.RMSECV;
   Q2_max(i)=CV.Q2_max;   
   R(i)=CV.R; % add by Guoyu Ding. 2022.7.6
   Rpc(i)=CV.optPC;
